...
def _financial_observations(
    asset: Any, world: dict[str, Any], producer: dict[str, Any]
) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "quality": {
                    "status": "calculated" if metric == "operating_margin" else "observed",
                    "confidence": 1.0,
                },
            }
        )
...
def _price_rows(
    spec: WorldSpec, result: Any, world: dict[str, Any], producer: dict[str, Any]
) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "provenance": {
                    "producer": producer,
                    "artifact": "public/prices.json",
                    "source_record": f"bar:{asset.ticker}:{index}",
                },
            }
        )
        ...
        aggregated.append(
            {
                **session_rows[-1],
                ...
                "provenance": {
                    "producer": producer,
                    "artifact": "public/prices.json",
                    "source_record": f"session:{asset.ticker}:{session}",
                },
            }
        )
    return aggregated
...
def export_world(output: Path, world_id: str = "test-world-001", seed: int = 42) -> Path:
    ...
    producer = {
        ...
        "git_sha": _git_sha(),
        "seed": seed,
        "config_hash": spec.specification_hash(),
    }
    ...
    for relative in (
        ...
        artifacts[relative] = {
            ...
            "visibility": "hidden" if relative.startswith("hidden/") else "public",
        }
    manifest = {
        "schema_version": "1",
        "world": world,
        ...
        "assets": entities,
        "artifacts": {
            "public": [key for key in artifacts if key.startswith("public/")],
            "hidden": [key for key in artifacts if key.startswith("hidden/")],
        },
