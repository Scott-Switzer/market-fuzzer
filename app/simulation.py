...
from app.agents.behaviors import AgentContext, BaseAgent, ExecutionAgent, MarketMaker, build_agents
...
def run_simulation(spec: WorldSpec) -> SimulationResult:
    ...
    for agent in agents:
        exchange.register(Account(agent.agent_id, agent.capital_cents, {asset.ticker: 0 for asset in spec.assets}))
    ...
    for agent in agents:
        if isinstance(agent, MarketMaker):
            for asset in spec.assets:
                context = AgentContext(0, asset.ticker, fundamentals[asset.ticker], prices[asset.ticker][-1],
                                       prices[asset.ticker], 0, liquidity_multiplier)
    ...
    for step in range(spec.clock.steps):
        for event in event_map.get(step, []):
            if event.asset:
                fundamentals[event.asset] = max(1, round(fundamentals[event.asset] * (1 + event.fundamental_effect_pct / 100)))
            else:
                for symbol in fundamentals:
                    fundamentals[symbol] = max(1, round(fundamentals[symbol] * (1 + event.fundamental_effect_pct / 100)))
            liquidity_multiplier *= event.liquidity_effect
            events_log.append({**event.model_dump(mode="json"), "step": step})

        factor = rng.gauss(0, {"low": 0.0002, "normal": 0.0005, "elevated": 0.001, "crisis": 0.002}[spec.macro.volatility_regime])
        for asset in spec.assets:
            ...
            fundamentals[asset.ticker] = max(1, round(previous * (1 + change) + asset.mean_reversion * (anchor - previous)))
        ...
        for agent in agents:
            ...
            for asset in spec.assets:
                ...
                mid = round((bid + ask) / 2) if bid is not None and ask is not None else (book.last_price_ticks or prices[asset.ticker][-1])
                context = AgentContext(step, asset.ticker, fundamentals[asset.ticker], mid, prices[asset.ticker],
                                       observed_volume, liquidity_multiplier)
        ...
        for asset in spec.assets:
            ...
            if book.last_price_ticks and abs(book.last_price_ticks / reference - 1) * 100 >= spec.exchange.circuit_breaker_pct:
                book.halt(step, spec.exchange.halt_steps)
            asset_states[asset.ticker] = {
                "mid_ticks": mid, "fundamental_ticks": fundamentals[asset.ticker],
                ...
                "volume": sum(row["quantity"] for row in exchange.trade_log[trade_cursor:] if row["symbol"] == asset.ticker),
                "halted": book.is_halted(step), "book": snapshot,
            }
        timeline.append({"step": step, "asset_states": asset_states, "liquidity_multiplier": liquidity_multiplier,
        ...
        for agent in agents:
            ...
            agent_states.append({"step": step, "agent_id": agent.agent_id, "agent_type": agent.agent_type,
                                 "cash_cents": account.cash_cents, "inventory": dict(account.inventory)})

    execution = next(agent for agent in agents if isinstance(agent, ExecutionAgent))
    strategy_trades = [trade for trade in exchange.trade_log if trade["buyer_id"] == execution.agent_id or trade["seller_id"] == execution.agent_id]
    ...
    summary = {
        "filled_quantity": executed, "fill_rate": executed / spec.experiment.parent_order.quantity,
        "average_execution_price_ticks": average, "arrival_price_ticks": arrival_price,
        "implementation_shortfall_bps": shortfall, "remaining_inventory": spec.experiment.parent_order.quantity - executed,
        "persistent_impact_bps": direction * (last_price / arrival_price - 1) * 10_000,
        "final_price_ticks": last_price, "total_market_volume": sum(trade["quantity"] for trade in exchange.trade_log),
        "fees_cents": exchange.fee_account_cents,
    }
    deterministic = {"spec_hash": spec.specification_hash(), "timeline": timeline, "orders": exchange.order_log,
                     "trades": exchange.trade_log, "events": events_log, "summary": summary}
    result_hash = hashlib.sha256(json.dumps(deterministic, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    return SimulationResult(spec.specification_hash(), result_hash, spec.seed, timeline, exchange.order_log,
                            exchange.trade_log, events_log, agent_states, summary, (time.perf_counter() - started) * 1_000)
