...
class TestTransactionCostModel:
    def test_short_inventory_borrow_fee_applied(self) -> None:
        ...
        flat_model = TransactionCostModel(
            spread_bps=2.0, borrow_fee_bps=0.0, impact_beta=0.0, default_adv=100_000.0
        )
        borrow_model = TransactionCostModel(
            spread_bps=2.0, borrow_fee_bps=10.0, impact_beta=0.0, default_adv=100_000.0
        )
        flat_curve = compute_equity_curve(prices, positions, tcost_model=flat_model, default_adv=100_000.0)
        borrow_curve = compute_equity_curve(
            prices, positions, tcost_model=borrow_model, default_adv=100_000.0
        )
    ...
    def test_sell_signal_with_borrow_fee(self) -> None:
        ...
        tcost = TransactionCostModel(
            spread_bps=2.0, borrow_fee_bps=12.0, impact_beta=0.0, default_adv=100_000.0
        )
        curve = compute_equity_curve(prices, positions, tcost_model=tcost, default_adv=100_000.0)
        no_borrow = TransactionCostModel(
            spread_bps=2.0, borrow_fee_bps=0.0, impact_beta=0.0, default_adv=100_000.0
        )
    ...
    def test_high_participation_nonlinear_impact(self) -> None:
        ...
        tcost = TransactionCostModel(
            spread_bps=2.0, borrow_fee_bps=0.0, impact_beta=0.5, impact_mode="sqrt", default_adv=10_000.0
        )
        ...
        tcost_linear = TransactionCostModel(
            spread_bps=2.0, borrow_fee_bps=0.0, impact_beta=0.5, impact_mode="linear", default_adv=10_000.0
        )
        linear_large = tcost_linear.costs_for_signals(
            prices.tolist(), large_trades.tolist(), default_adv=10_000.0
        )
    ...
    def test_backward_compatible_when_disabled(self) -> None:
        ...
        flat_model = TransactionCostModel(
            spread_bps=4.0, borrow_fee_bps=0.0, impact_beta=0.0, default_adv=100_000.0
        )
        explicit = backtest_metrics(prices, positions, tcost_model=flat_model, default_adv=100_000.0)
        curve_explicit = compute_equity_curve(
            prices, positions, tcost_model=flat_model, default_adv=100_000.0
        )
