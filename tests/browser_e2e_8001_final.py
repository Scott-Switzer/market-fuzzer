    ...
    with sync_playwright() as p:
        ...
        def on_console(msg):
            ...
            if "polygon" in lowered or "points:" in lowered:
                return
            console_errors_local.append(text)
        ...
        with page.expect_response(
            lambda r: r.url.endswith("/api/break-test/run") and r.status == 200, timeout=180000
        ) as resp:
            page.click("#run-btn")
        run_json = resp.value.json()
        assert isinstance(run_json, dict) and any(
            k in run_json for k in ("historical", "equity_curve", "forward_test")
        ), run_json.keys()
        page.wait_for_timeout(500)
        ...
        oos_r = httpx.post(
            "http://127.0.0.1:8001/api/quant/oos",
            json={
