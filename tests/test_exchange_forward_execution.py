...
CLOSES = [
    100.0, 100.5, 101.2, 101.8, 102.4, 103.1, 103.5, 104.0, 104.7, 105.3,
    106.1, 106.5, 106.9, 107.4, 108.2, 108.6, 109.1, 109.5, 110.0, 110.4,
    110.9, 111.3, 112.0, 112.4, 112.8, 113.2, 113.6, 114.1, 114.5, 115.0,
    115.4, 115.8, 116.2, 116.7, 117.1, 117.5, 117.9, 118.4, 118.8, 119.2,
    119.6, 120.0, 120.4, 120.8, 121.2, 121.6, 122.0, 122.4, 122.8, 123.2,
    123.6, 124.0, 124.4, 124.8, 125.2, 125.6, 126.0, 126.4, 126.8, 127.2,
    127.6, 128.0, 128.4, 128.8, 129.2, 129.6, 130.0, 130.4, 130.8, 131.2,
    131.6, 132.0, 132.4, 132.8, 133.2, 133.6, 134.0, 134.4, 134.8, 135.2,
    135.6, 136.0, 136.4, 136.8, 137.2, 137.6, 138.0, 138.4, 138.8, 139.2,
    139.6, 140.0, 140.4, 140.8, 141.2, 141.6, 142.0, 142.4, 142.8, 143.2,
    143.6, 144.0, 144.4, 144.8, 145.2, 145.6, 146.0, 146.4, 146.8, 147.2,
    147.6, 148.0, 148.4, 148.8, 149.2, 149.6, 150.0, 150.4, 150.8, 151.2,
]
...
class TestExchangeForwardRouter:
    def test_router_requires_target_symbol_and_side(self) -> None:
        with pytest.raises(ValueError):
            UserStrategyOrderRouter(target_symbol="SYNTH", side="x", strategy_type="sma_crossover", params=PARAMS)
        with pytest.raises(ValueError):
            UserStrategyOrderRouter(target_symbol="SYNTH", side="buy", strategy_type="sma_crossover", params=PARAMS, order_type="unknown")

    def test_hold_when_remaining_parent_quantity_is_exhausted(self) -> None:
        router = UserStrategyOrderRouter(target_symbol="SYNTH", side="buy", strategy_type="sma_crossover", params=PARAMS)
        action = router.decide({"symbol": "SYNTH", "step": 0, "inventory": 0, "remaining_quantity": 0, "recent_prices": CLOSES[:20]})
        assert action["action_type"] == "hold"

    def test_submit_action_carries_required_fields(self) -> None:
        router = UserStrategyOrderRouter(target_symbol="SYNTH", side="sell", strategy_type="sma_crossover", params=PARAMS, base_quantity=100)
        action = router.decide({"symbol": "SYNTH", "step": 35, "inventory": 0, "remaining_quantity": 5_000, "recent_prices": CLOSES[:40]})
    ...
    def test_decider_wraps_action_for_simulation(self) -> None:
        ...
        action = decider({"symbol": "SYNTH", "step": 35, "inventory": 0, "remaining_quantity": 100, "recent_prices": CLOSES[:40]})
...
class TestExchangeForwardExecution:
    ...
    def test_synthetic_fills_exist_for_multiple_regimes(self, strategy_type: str, params: dict[str, int]) -> None:
    ...
    def test_same_seed_returns_same_fill_sequence(self) -> None:
        ...
        for left, right in zip(ids1, ids2):
            assert left == right
